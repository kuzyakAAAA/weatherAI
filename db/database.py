...
class User(Base):
    ...
    # удобное текстовое представление объекта пользователя
    # помогает при отладке, выводе в консоль и логах
    def __repr__(self):
        return f"<User(user_id={self.user_id}, style='{self.style}')>"
...
class History(Base):
    ...
    # удобное текстовое представление объекта истории
    # помогает быстро понять, какая запись была получена из базы данных
    def __repr__(self):
        return f"<History(id={self.id}, user_id={self.user_id}, city='{self.city}')>"
